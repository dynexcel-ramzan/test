...
class emp_payroll_statement(models.TransientModel):
    ...
    def get_payslip(self):
        structure_payslip_id = self.env['hr.payslip'].search(
            [('struct_id', '=', self.structure_id.id), ('employee_id', '=', self.employee_ids.ids)])
    ...
    def print_excel(self):
        ...
        fp = BytesIO()
        workbook.save(fp)
        export_id = self.env['emp.payroll.statement.excel'].create(
