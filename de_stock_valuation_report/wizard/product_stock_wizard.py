...
class ProductStock(models.TransientModel):
    ...
    product_id = fields.Many2one('product.product', string='Product', required=False,
                                 help='Select Product for movement')
    categ_ids = fields.Many2many('product.category', string='Categories', required=True)
    ...
    def print_report(self):
        data = {'product_id': self.product_id.id, 'categ_ids': [p.id for p in self.categ_ids],
                'pricelist_id': self.pricelist_id.id, 'dated': self.dated, 'warehouse_id': self.warehouse_id.id,
