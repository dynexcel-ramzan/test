...
and l.id = %(location_id)s
    ...
    def _get_saleprice(self, categ_id, product_tmpl_id, product_id, pricelist_id, list_price):
        ...
        variant_pricelist_item = self.env['product.pricelist.item'].search(
        ...
        if not variant_pricelist_item:
            product_pricelist_item = self.env['product.pricelist.item'].search(
                [('pricelist_id', '=', pricelist_id), ('product_tmpl_id', '=', product_tmpl_id)], limit=1)
            ...
            if not product_pricelist_item:
                categ_pricelist_item = self.env['product.pricelist.item'].search(
                    [('pricelist_id', '=', pricelist_id), ('categ_id', '=', categ_id)], limit=1)
                pricelist_item = categ_pricelist_item

        base_pricelist_item = self.env['product.pricelist.item'].search(
            [('pricelist_id', '=', pricelist_item.base_pricelist_id.id), ('product_tmpl_id', '=', product_tmpl_id)],
            limit=1)
        ...
        for pitem in pricelist_item:
            if pitem.compute_price == 'fixed':
                ...
                saleprice = pitem.product_tmpl_id.list_price - (
                            pitem.product_tmpl_id.list_price * (pitem.percent_price / 100))
            else:
                saleprice = pitem.product_tmpl_id.list_price
