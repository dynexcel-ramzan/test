...
class VoucherVoucher(models.Model):
	...
	def get_default_values(self, field=False):
		...
		vals = {}
		if field:
			if field == 'wk_coupon_customer_type' and self._context.get('is_template'):
				return False
	...
	is_template = fields.Boolean(string='Is Voucher Template', default=lambda self: self._get_is_template())
	validity_in_days = fields.Integer(string='Validity In Days', default=0)
	voucher_ids = fields.Many2many('voucher.voucher', 'voucher_template', 'voucher_id', 'parent_voucher_id', string='Vouchers')
	...
	def create(self, vals):
		...
		if vals.get('expiry_date'):
			...
			if int(diff) >= 0:
				diff += 1
			vals['validity'] = diff
		if not is_template and str(vals.get('expiry_date')) < vals.get('issue_date'):
		...
		if not is_template and (vals.get('expiry_date') and datetime.now().date() > datetime.strptime(str(vals['expiry_date']),'%Y-%m-%d').date()):
			raise  ValidationError(('Expiry date have been passed already. Please select a correct expiry date..'))
		if not is_template and (vals.get('issue_date') and  datetime.now().date() > datetime.strptime(str(vals.get('issue_date')),'%Y-%m-%d').date()):
		...
		if is_template:
			vals.update(is_template=True, active=False, validity_in_days=abs(vals.get('validity_in_days') or 1))
		res  = super(VoucherVoucher, self).create(vals)
		if not res.is_template:
			history_values = {
				'name':res.name,
				'create_date':datetime.now(),
				'voucher_value':res.voucher_value,
				'transaction_type':'credit',
				'channel_used':res.voucher_usage,
				'voucher_id':res.id
				}
	...
	def write(self, vals):
		for voucher in self:
			...
			if vals.get('validity_in_days'):
				vals.update(validity_in_days=abs(vals.get('validity_in_days')))
	...
	def action_get_related_voucher(self):
		self.ensure_one()
		action = self.env.ref("wk_coupons.voucher_voucher_action_tree").sudo().read()[0]
		action['domain'] = [('id', 'in', self.voucher_ids.ids)]
		return action


	def generate_voucher_from_template(self, template_id):
		# @param template_id: voucher object has is_template filed is true
		template = self.search([('id', '=', int(template_id)), ('is_template', '=', True), ('active', '=', False)]).sudo()
		if template.exists():
			expiry_date = fields.Date.today()+fields.date_utils.relativedelta(days=(template.validity_in_days or 1)-1)
			voucher = template.copy({
				'is_template': False,
				'active': True,
				'issue_date': fields.Date.today().strftime('%Y-%m-%d'),
				'expiry_date': expiry_date.strftime('%Y-%m-%d')
			})
			template.voucher_ids |= voucher
			return voucher
		return False
