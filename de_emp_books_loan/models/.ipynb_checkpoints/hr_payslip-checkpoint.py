...
class HRPayslip(models.Model):
    ...
    def action_payslip_done(self):
        for payslip in self:
            ...
            loan_line_ids = self.env['hr.employee.loan.line'].search([('employee_id', '=', payslip.employee_id.id),                                                         ('state', '=', 'done'),('date_due','>=',payslip.date_from),('date_due','<=',payslip.date_to)])
            ...
            for loan in loan_line_ids:
                ...
                self.env['hr.employee.loan.disburse'].create({
                    'name': _('Disbursed %s %s - %s - %s', loan.employee_loan_id.name,formatLang(self.env, loan.amount_residual, currency_obj=loan.currency_id),payslip.employee_id.name,format_date(self.env, fields.Date.to_string(fields.Date.today()))),
