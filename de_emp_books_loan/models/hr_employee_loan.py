...
class HREmployeeLoanRule(models.Model):
    ...
    rule_basis = fields.Selection([
        ...
        ('close', 'Closed'),
        ('cancel', 'Refused')
    ], string='Status', copy=False, index=True, compute='_compute_state', store=True, default='draft',
        help="Status of the Avances.")
...
class HREmployeeLoan(models.Model):
    ...
    def _get_employee_id_domain(self):
        ...
        if self.user_has_groups('de_emp_books_loan.group_hr_loan_user') or self.user_has_groups(
    ...
    ref = fields.Char(string='Reference', required=True, copy=False, readonly=True, index=True,
                      default=lambda self: _('New'))

    date = fields.Date(readonly=True, states={'draft': [('readonly', False)], }, required=True,
                       default=fields.Date.context_today, string="Request Date")

    accounting_date = fields.Date(readonly=True, states={'draft': [('readonly', False)], },
                                  default=fields.Date.context_today, string="Accounting Date")
    ...
    employee_id = fields.Many2one('hr.employee', string="Employee", store=True, required=True, readonly=True,
    ...
    department_id = fields.Many2one('hr.department', compute='_compute_from_employee_id', store=True, readonly=True,
                                    copy=False, string='Department')
    ...
    loan_type_id = fields.Many2one('hr.employee.loan.type', string='Loan Type', readonly=True, copy=False,
                                   states={'draft': [('readonly', False)]})
    product_id = fields.Many2one('product.product', string='Product', compute='_compute_from_loan_type', store=True,
    ...
    state = fields.Selection([
        ...
        help="Status of the Loan.")

    amount = fields.Monetary("Amount", currency_field='currency_id', tracking=True, required=True, readonly=True,
                             copy=False, states={'draft': [('readonly', False)]})
    amount_residual = fields.Monetary("Residual Amount", currency_field='currency_id', tracking=True, required=True,
                                      readonly=True, copy=False, compute='_compute_all_amount')

    description = fields.Text('Notes...', readonly=True,
                              states={'draft': [('readonly', False)], 'submit': [('readonly', False)],
                                      'cancel': [('readonly', False)]})

    interest_rate = fields.Float(string='Interest Rate', required=True, readonly=True, copy=False,
                                 states={'draft': [('readonly', False)]})
    date_start = fields.Date(string='Start Date', required=True, readonly=True, copy=False,
                             states={'draft': [('readonly', False)]})
    date_end = fields.Date(string='End Date', store=True, compute='_compute_date_end', required=True, readonly=True,
                           copy=False, states={'draft': [('readonly', False)]})
    installments = fields.Integer(string='Installments', default=1, required=True, readonly=True, copy=False,
                                  states={'draft': [('readonly', False)]})

    address_id = fields.Many2one('res.partner', compute='_compute_from_employee_id', store=True, copy=True,
                                 string="Employee Home Address", check_company=True, readonly=False,
                                 states={'done': [('readonly', True)], 'close': [('readonly', True)],
                                         'cancel': [('readonly', True)]})

    journal_id = fields.Many2one('account.journal', string='Journal', domain="[('type','=','purchase')]",
                                 default=_default_journal_id, readonly=False,
                                 states={'done': [('readonly', True)], 'close': [('readonly', True)],
                                         'cancel': [('readonly', True)]})
    ...
    loan_deferred_ids = fields.One2many('hr.employee.loan.deferred', 'employee_loan_id', string='Loan Schedule',
    ...
    def approve_request(self):
        if not self.user_has_groups('de_emp_books_loan.group_hr_loan_team_approver'):
            ...
            if not self.env.user in current_managers and not self.user_has_groups(
                    'de_emp_books_loan.group_hr_loan_user') and self.employee_id.parent_id.user_id != self.env.user:
    ...
    def action_refuse_loan(self, reason):
        if not self.user_has_groups('de_emp_books_loan.group_hr_loan_team_approver'):
            ...
            if not self.env.user in current_managers and not self.user_has_groups(
                    'de_emp_books_loan.group_hr_loan_user') and self.employee_id.loan_manager_id != self.env.user:
                raise UserError(_("You can only refuse your department expenses"))

        self.write({'state': 'refused'})
        for loan in self:
            loan.message_post_with_view('de_emp_books_loan.hr_employee_loan_template_refuse_reason',
                                        values={'reason': reason, 'is_sheet': True, 'name': loan.name})
        self.activity_update()
            ...
            if allocation > 0:
                self.env['hr.employee.loan.disburse'].create({
                    'name': _('Disbursed %s %s - %s - %s', loan.employee_loan_id.name,
                              ...
                              self.employee_id.name, format_date(self.env, fields.Date.to_string(fields.Date.today()))),
...
class HREmployeeLoanLine(models.Model):
    ...
    state = fields.Selection([
        ...
        help="Status of the monthly loan line.")

    loan_line_disburse_ids = fields.One2many('hr.employee.loan.disburse', 'employee_loan_line_id',
                                             string='Loan Schedule', copy=False, readonly=True,
                                             states={'draft': [('readonly', False)]})

    @api.depends('employee_loan_id', 'employee_loan_id.state', 'loan_line_disburse_ids',
                 'loan_line_disburse_ids.amount_residual', 'loan_line_disburse_ids.payslip_state')
    def _compute_state(self):
        status = 'draft'
        for loan in self:
...
class HREmployeeLoanDisburse(models.Model):
    ...
    name = fields.Char()
    employee_loan_line_id = fields.Many2one('hr.employee.loan.line', string="Employee Loan Line", readonly=True,
                                            copy=False)
